 ...
 */   
    ...
    app.controller('meniuPctrl',function($rootScope,$scope,$filter){
       ...
       /*  $scope.viuuVizibil={locatii:false
         ...
         $scope.inputListaSA=function(){
            var arr=$filter('filter')($rootScope.vDatele.crtDatele.nume,$scope.mListSA)
            if (arr.length==0) $rootScope.vv.butonPlus=true
                        else $rootScope.vv.butonPlus=false
         }
        ...
        $scope.apasPlus=function(){
          ...
          */  
            $rootScope.vDatele.crtDatele.nume.unshift($scope.mListSA)
            $scope.mListSA=''
            $rootScope.vv.butonPlus=false
            
         }
        $scope.apasButonMeniu=function(){
             $rootScope.vv.meniu=!$rootScope.vv.meniu
         }
    ...
    app.controller('dateleCtrl',function($scope,$rootScope){
       // $rootScope.mListSA=''
        $scope.crtClasa='element'
        $rootScope.vDatele={crtDatele:{}}
                   /*   ,locatii:{nume:[],tip:[]}
                      ,musteri:{nume:[],tip:[]}
                      ,furnizori:{nume:[],tip:[]}
                     }*/
        
        $scope.setcrtDatele=function(lista){
            $rootScope.vDatele.crtDatele=$rootScope.vDatele[lista]
        }
        $scope.adaugElmToCrtDatele=function(elm){
            
        }
        var oRef=$rootScope.vDatele
        var dateleInitial=function(){
             oRef['locatii']={nume:[] ,tip:[]}
             arrLocatii.forEach(function(e,i){
                 oRef.locatii.nume[i]=e
                 //oRef.locatii.tip[i]='element'
             })
             $scope.setcrtDatele('locatii')
        }
        dateleInitial()
    })
  
     
     app.controller('unuTitlu',function($scope,$rootScope){
         $scope.elmApasat=function(idx){
    ...
    app.controller('manageViuu',function($rootScope){
        $rootScope.vv={initial:false
                       ,barTitlu:false
                       ,meniu:false
                       ,fvi:false
                       ,locatii:false
        ...
        var deschide={initial:['barTitlu','inpItems','fvi','rfitems']
                     ,locatii:['barTitlu','inpListSA','locatii']
                     ,activitati:['barTitlu','inpAT']
                     ,itemPage:[]
                     ,fvi:['barTitlu','inpItems','rfitems']
                     }
        var setVVinitial=function(){
                for (var k in $rootScope.vv) $rootScope.vv[k]=false
        }
                     
        $rootScope.dsc=function(kd){
            setVVinitial()
            $rootScope.vv[kd]=true
            deschide[kd].forEach(function(e){
                            $rootScope.vv[e]=true
            })
...
var crtModul='itemPage' //'locatii'
    
    app.service('setViuu',function(){
     var viuuNume={initial:false
                   ,barTitlu:false
                   ,meniu:false
                   ,fvi:false
                   ,locatii:false
     ...
     var deschide={initial:['barTitlu','inpItems','fvi']
                     ,locatii:['barTitlu','inpListSA']
                     ,activitati:['barTitlu','inpAT']
                     ,itemPage:[]
                     ,fvi:['barTitlu','inpItems','rfitems']
                     }
    ...
    var tabelFiltrare=new fTabOavdAim(fbs_oAvdAim)
    var cladescFVI=function(){ }
    
   
   
    app.service('initDatele',function(){
        var arez ,aista=this
        this.aduDatele=function(numeModul){
            switch (numeModul) {
                case 'locatii':
                    return oLocatii
                    break;
                case 'itemPage':
                        var ot={arrNumeFoldare:[],idxFdeschis:0}
                        ot.arrNumeFoldare=initiereDateLista(itemPagina)
                        return cladescFolderLista(ot)
                    break;
            }
        }
    })
    
    app.service('dateleServ',function(initDatele){
           var primaOara={locatii:true
                          ,itemPage:true
                         }
           var setInitial=function(numeModul){
                   sDatele[numeModul]=initDatele.aduDatele(numeModul)
               }
       var sDatele={}
        
        this.getDatele=function(numeModul){
            if (primaOara[numeModul]) setInitial(numeModul)
            switch (numeModul) {
                case 'itemPage':
                   
                break;
            }
            return sDatele[numeModul]
        }
        this.updateDatele=function(numeModul){
            switch (numeModul){
                case 'itemPage':
                  //  cladescFolderLista(activFolder,sDatele[numeModul])
                break;
            }
        }
        this.salveazaDatele=function(numeModul,datele){
               sDatele[numeModul]=datele
            }
 ...
 /*   app.service('actUserServ',function(dateleServ){
       // dateleServ.modulNou('locatii',{items:['crtLoc','totLoc'],tip:['element','element']})
       // var d=dateleServ.getDatele(crtModul)
        this.crtDatele={}
        this.actiune={
                locatii:function(idx){ 
                          var x=dateleServ.sDatele[crtModul].prp[idx].tip
                            switch (x) {
                               case 'titlu':
      ...
      //  this.getDatele=function(numeModul){
      // return dateleServ.getDatele(numeModul)
      //  }
    ...
    app.controller('listaInpCtrl',function($scope,dateleServ){
        $scope.titlu=crtModul
         var getDatele=function(){
                 $scope.vDatele=dateleServ.getDatele(crtModul)
             }
         $scope.elmApasat=function(idx,e){
            // actUserServ.modulul[crtModul].elmApasat(idx)
              
              actiune[crtModul](idx.vDatele.prp)
         }
         $scope.getPrp=function(numeProprietate,idx){
             return $scope.vDatele.prp[idx][numeProprietate]
         }
       /*  $scope.testare=function(){
             $scope.elmApasat(0)
         } */
         getDatele()
     })
